...
- **Web3Forms delivers to `magicshine2601@gmail.com`** — set in the dashboard on
  2026-08-28 and recorded in `SITE_DETAILS.formRecipient`. The payload cannot name a
  recipient (that is what stops the service being an open relay), so those two have to be
  changed together, and they will have to be changed together again when the temporary
  Gmail address gives way to a mailbox on the real domain.
